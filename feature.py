...
class TopScores(object):
    ...
    def __init__(self, size=100):
        ...
        self.size    = size
        self.min     = -(sys.maxint-1)             # current min score
        self.list    = [(self.min, None)] * size   # initialize list to tuples with min
    ...
    def insert(self, site, score):
        ...
        if score > self.min:
            ...
            self.list = sorted(self.list,reverse=True)[0:self.size] # tuples sort on 1st elt
            self.min = self.list[self.size-1][0]                    # trim the list
...
class Protein(object):
    ...
    def getProtein(filename, path):
        ...
        if not Protein.proteins.has_key(filepath):
    ...
    def __init__(self, filepath):
        ...
        self.alpha_carbons = []
        self.min = Point(sys.maxint, sys.maxint, sys.maxint)
        self.max = Point(-(sys.maxint-1),-(sys.maxint-1),-(sys.maxint-1))
        self._loadFile()
    ...
    def _loadFile(self):
        ...
        for line in open(self.filepath):
            ...
            point     = Point(float(line[30:38]), float(line[38:46]), float(line[46:54]))
...
class Feature(object):
    ...
    def __init__(self, sites_file, nonsites_file, path="."):
        ...
        if verbose:
            print "len(site_counts)=%d num_sites=%d" % (len(site_counts), num_sites)
            print "site_counts=%s" % ",".join([str(i) for i in site_counts])
            print "len(nonsite_counts)=%d num_nonsites=%d" % (len(nonsite_counts), num_nonsites)
        ...
        self.pr_in_site = (site_counts + Feature.NUMERATOR_EPSILON) / \
                (Vector([num_sites]*20*5) + Feature.DENOMINATOR_EPSILON)
        ...
        self.pr_in_nonsite = (nonsite_counts + Feature.NUMERATOR_EPSILON) / \
                (Vector([num_nonsites]*20*5) + Feature.DENOMINATOR_EPSILON)
    ...
    def score(self, site):
        ...
        scores = [_scoreFeature(index, value) for index, value in enumerate(features)]
    ...
    def _sitesInFile(self, sites_filename):
        ...
        for line in open(sites_filename):
            ...
            site = Site(Protein.getProtein(protein_filename, self.path), Point(x,y,z))
            yield site
...
class OutputWriter(object):
    ...
    def __init__(self, feature, outputdir):
        ...
        formats          = [Format.float3strip,Format.float3strip,Format.float3strip,Format.float3]
        ...
        for shell in range(5):
            ...
            for index, residue in enumerate(RESIDUES):
                output.write("\t".join(["%s" % residue,
                    "%.3f" % self.feature.pr_in_site[20*shell+index],
    ...
    def outputScoresForFile(self, sites_filename):
        ...
        output_filename = OutputWriter.filename_append(sites_basename, "_scores")
        output_filepath = os.path.join(self.outputdir, output_filename)
        output = open(output_filepath, "w")
    ...
    def outputScoresForProteinFile(self, protein_filename):
        ...
        output_filename = OutputWriter.filename_append(protein_basename, "_100", ".txt")
        output_filepath = os.path.join(self.outputdir, output_filename)
        output = open(output_filepath, "w")
...
def main(argv):
    ...
    #    feature.py -s sites.txt -n nonsites.txt -d pdbdir -p protein.pdb -o outputdir
    global verbose
    ...
    if verbose:
        print "Loading sites files (%s, %s, %s)..." % (sites_filename, nonsites_filename, pdb_directory)
    ...
    output_writer.formats = [Format.float3strip,Format.float3strip,Format.float3strip,Format.float3]
    ...
    output_writer.formats = [Format.float,Format.float3strip,Format.float3strip,Format.float3]
    ...
    if protein_filename:
        ...
        if verbose:
            print "Writing out top 100 scores for file '%s' ..." % protein_filename

        output_writer.formats = [Format.float3,Format.float3,Format.float3,Format.float3]
